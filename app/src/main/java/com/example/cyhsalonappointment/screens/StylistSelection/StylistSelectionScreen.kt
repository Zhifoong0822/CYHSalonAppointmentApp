        ...
        LazyColumn {
            items(stylists) { stylist ->
                Card(
                    ...
                    border = if (selectedStylistId == stylist.stylistID) BorderStroke(2.dp, MaterialTheme.colorScheme.primary) else null
                ) {
                    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Button(
            onClick = {
                if (selectedStylistId != null) {
                    // Navigate to temporary payment screen
                    navController.navigate(
                        "tempPayment/$serviceName/$selectedDate/$selectedTimeSlot/$selectedStylistId"
                    )

                }
            },
            enabled = selectedStylistId != null,
